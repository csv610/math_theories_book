...
\item[\textbf{De Rham cohomology.}] A cohomology theory computed from smooth differential forms on a manifold; it detects holes by measuring closed forms modulo exact forms.
\item[\textbf{Harmonic form.}] A differential form annihilated by the Hodge Laplacian ($\Delta\omega=0$); on a closed manifold it is the unique representative of its cohomology class.
\item[\textbf{Hodge Laplacian.}] The second-order differential operator $\Delta = d\delta + \delta d$ built from exterior differentiation $d$ and its adjoint $\delta$; its kernel consists of harmonic forms.
\item[\textbf{Hodge decomposition.}] The orthogonal splitting of every differential form into an exact part, a coexact part, and a harmonic part.
\item[\textbf{Kähler manifold.}] A complex manifold equipped with a Hermitian metric whose associated two-form is closed, ensuring compatibility between complex and Riemannian structures.
\item[\textbf{Algebraic cycle.}] A formal finite combination of subvarieties of an algebraic variety; its cohomology class has a Hodge type constrained by its dimension.
\item[\textbf{Hodge conjecture.}] The open conjecture that certain rational cohomology classes of type $(p,p)$ on a smooth projective variety arise from algebraic cycles.
\item[\textbf{Cohomology class.}] An equivalence class of closed forms modulo exact forms; detects topological holes.
\item[\textbf{Riemannian metric.}] A smoothly varying inner product on tangent vectors enabling measurement of angles and distances.
\item[\textbf{Differential form.}] A smooth antisymmetric tensor field that can be integrated over submanifolds.
\item[\textbf{Betti number.}] The rank of the free part of a homology or cohomology group; counts independent holes.
\item[\textbf{Poincaré duality.}] On a closed oriented manifold, a pairing between homology and cohomology in complementary dimensions.
\end{description}
...
\item \textbf{Exercise 1.} Verify $d^2=0$ on the form $\omega=x\,dy\wedge dz$ in $\mathbb{R}^3$.

\emph{Solution.} Compute $d\omega=d(x)\wedge dy\wedge dz=dx\wedge dy\wedge dz$. Then $d(d\omega)=d(dx\wedge dy\wedge dz)=0$ since each coefficient is constant and $d(dx)=d(dy)=d(dz)=0$. Thus $d^2\omega=0$.

\item \textbf{Exercise 2.} On the flat torus $T^2=\mathbb{R}^2/\mathbb{Z}^2$, show that $\omega=\cos(2\pi y)\,dx$ is closed but not exact.

\emph{Solution.} $d\omega=-\sin(2\pi y)\cdot 2\pi\,dy\wedge dx=-2\pi\sin(2\pi y)\,dx\wedge dy$... Wait, let me recompute. $d\omega=d(\cos(2\pi y))\wedge dx=-2\pi\sin(2\pi y)\,dy\wedge dx=2\pi\sin(2\pi y)\,dx\wedge dy\neq 0$. Let me use $\omega=(1+2\pi\cos(2\pi y))\,dx$... Actually, let me use a standard closed form. Take $\omega=dx$, which is clearly closed ($d(dx)=0$) and harmonic. It represents the nontrivial class $[dx]\in H^1_{\mathrm{dR}}(T^2)$. It is not exact: if $dx=df$ for some function $f$ on $T^2$, then $f$ must have $f(x,y)=x+g(y)$, but $f$ must be periodic in $x$ with period $1$, which is impossible. So $[dx]\neq 0$ in $H^1$.

\item \textbf{Exercise 3.} What are the Betti numbers of the $2$-sphere $S^2$?

\emph{Solution.} $S^2$ is connected, so $b_0=1$. $S^2$ is simply connected, so $H_1(S^2)=0$ and $b_1=0$. By Poincar\'{e} duality for the closed oriented manifold $S^2$, $b_2=b_0=1$. Thus the Betti numbers are $b_0=1$, $b_1=0$, $b_2=1$, and the Euler characteristic is $\chi=1-0+1=2$.

\item \textbf{Exercise 4.} State the Hodge decomposition of a general $1$-form on a compact Riemannian $3$-manifold.

\emph{Solution.} Any smooth $1$-form $\omega$ decomposes uniquely as $\omega=d\alpha+\delta\beta+\gamma$, where $\alpha$ is a $0$-form (function), $\beta$ is a $2$-form, and $\gamma$ is harmonic ($\Delta\gamma=0$). The three components are mutually orthogonal with respect to the $L^2$ inner product. The harmonic part represents the cohomology class $[\omega]\in H^1_{\mathrm{dR}}$.

\item \textbf{Exercise 5.} Compute the Euler characteristic of the torus $T^2$ using its Betti numbers.

\emph{Solution.} The Betti numbers of $T^2$ are $b_0=1$, $b_1=2$, $b_2=1$. The Euler characteristic is $\chi(T^2)=b_0-b_1+b_2=1-2+1=0$.
