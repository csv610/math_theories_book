...
\item \textbf{Exercise 1.} Use the Routh--Hurwitz criterion to determine whether all roots of $s^3+3s^2+5s+7=0$ have negative real part.

\emph{Solution.} The Routh array is: $s^3$: $1$, $5$; $s^2$: $3$, $7$; $s^1$: $(3\cdot5-1\cdot7)/3=8/3$; $s^0$: $7$. All first-column entries ($1,3,8/3,7$) are positive, so all roots have negative real part. The system is stable.

\item \textbf{Exercise 2.} Find the closed-loop poles of the transfer function $G(s)=1/(s^2+5s+6)$ and determine stability.

\emph{Solution.} The characteristic equation is $s^2+5s+6=0$, which factors as $(s+2)(s+3)=0$. The poles are $s=-2$ and $s=-3$. Both lie in the open left half-plane, so the system is asymptotically stable.

\item \textbf{Exercise 3.} Check controllability of the system $\dot x=\begin{bmatrix}0&1\\-2&-3\end{bmatrix}x+\begin{bmatrix}0\\1\end{bmatrix}u$.

\emph{Solution.} The controllability matrix is $\mathcal C=[B\;AB]=\begin{bmatrix}0&1\\1&-3\end{bmatrix}$. Its determinant is $0\cdot(-3)-1\cdot1=-1\neq0$. Since $\mathcal C$ has full rank, the system is controllable.

\item \textbf{Exercise 4.} A discrete-time system has transfer function $H(z)=1/(z-0.5)$. Is it stable?

\emph{Solution.} The pole is at $z=0.5$, which lies strictly inside the unit circle ($|0.5|<1$). The impulse response is $h[n]=(0.5)^n u[n]$, which decays to zero. The system is asymptotically stable.

\item \textbf{Exercise 5.} For the system $\dot x=\begin{bmatrix}1&0\\0&-2\end{bmatrix}x+\begin{bmatrix}1\\1\end{bmatrix}u$, $y=\begin{bmatrix}1&0\end{bmatrix}x$, determine whether the system is both controllable and observable.

\emph{Solution.} Controllability: $\mathcal C=[B\;AB]=\begin{bmatrix}1&1\\1&-2\end{bmatrix}$, $\det\mathcal C=-3\neq0$, so controllable. Observability: $\mathcal O=\begin{bmatrix}C\\CA\end{bmatrix}=\begin{bmatrix}1&0\\1&0\end{bmatrix}$, $\operatorname{rank}\mathcal O=1<2$, so not observable. The second state is unobservable because $C$ cannot distinguish its contribution.
