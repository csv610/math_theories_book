...
\item \textbf{Exercise 1.} Let $X=\mathbb{R}$ with the usual topology and let $\mathcal{F}$ be the sheaf of continuous real-valued functions. Let $U_1=(-\infty,2)$ and $U_2=(1,\infty)$. Define $f_1(x)=x$ on $U_1$ and $f_2(x)=x^2-3x+4$ on $U_2$. Do $f_1$ and $f_2$ agree on $U_1\cap U_2=(1,2)$? If so, describe the unique global section $f\in\mathcal{F}(\mathbb{R})$ they glue to. \emph{Solution.} On $(1,2)$: $f_1(x)=x$ and $f_2(x)=x^2-3x+4$. At $x=1.5$: $f_1=1.5$, $f_2=2.25-4.5+4=1.75\ne 1.5$. These do not agree on the overlap. So $f_1$ and $f_2$ do not glue to a global continuous function.

\item \textbf{Exercise 2.} Let $X=\{a,b\}$ with the discrete topology. Let $\mathcal{F}$ be the presheaf where $\mathcal{F}(\{a,b\})=\mathbb{Z}$, $\mathcal{F}(\{a\})=\mathbb{Z}$, $\mathcal{F}(\{b\})=\mathbb{Z}$, and all restriction maps are the identity. Verify that $\mathcal{F}$ is a sheaf. \emph{Solution.} The only nontrivial cover is $\{a\}\cup\{b\}=\{a,b\}$. Sections $s_a\in\mathcal{F}(\{a\})$ and $s_b\in\mathcal{F}(\{b\})$ must agree on $\{a\}\cap\{b\}=\varnothing$ (vacuously true). The unique global section restricting to $s_a$ on $\{a\}$ and $s_b$ on $\{b\}$ is $s_a$ (which equals $s_b$ in $\mathbb{Z}$). This section is unique. The sheaf condition holds, so $\mathcal{F}$ is a sheaf.

\item \textbf{Exercise 3.} Let $X$ be a topological space and let $\mathcal{F}$ be the constant presheaf assigning $\mathbb{Z}$ to every nonempty open set with all restriction maps the identity. Show that if $X$ is disconnected (say $X=U\cup V$ with $U,V$ open, nonempty, disjoint), then $\mathcal{F}$ is not a sheaf. \emph{Solution.} Take the open cover $\{U,V\}$ of $X$. Choose sections $s_U=0\in\mathcal{F}(U)=\mathbb{Z}$ and $s_V=1\in\mathcal{F}(V)=\mathbb{Z}$. They agree on $U\cap V=\varnothing$ (vacuously). But there is no global section $s\in\mathcal{F}(X)=\mathbb{Z}$ restricting to both $0$ on $U$ and $1$ on $V$, since the restriction maps are identity. The sheaf condition fails.

\item \textbf{Exercise 4.} On $X=\mathbb{R}$, let $\mathcal{F}$ be the sheaf of continuous functions and $\mathcal{G}$ the sheaf of smooth functions. There is a natural inclusion $\iota\colon\mathcal{G}\hookrightarrow\mathcal{F}$. For the open set $U=(0,1)$, compute the cokernel sheaf $\mathcal{C}=\mathcal{F}/\mathcal{G}$ at $U$: i.e., find $\mathcal{C}(U)=\mathcal{F}(U)/\mathcal{G}(U)$. \emph{Solution.} $\mathcal{F}(U)=C^0((0,1))$ (continuous functions on $(0,1)$) and $\mathcal{G}(U)=C^\infty((0,1))$ (smooth functions on $(0,1)$). Since every smooth function is continuous but not every continuous function is smooth, $\mathcal{G}(U)\subsetneq\mathcal{F}(U)$. The quotient $\mathcal{C}(U)=C^0((0,1))/C^\infty((0,1))$ is an uncountably dimensional vector space: for instance, $f(x)=|x-1/2|$ is continuous but not smooth, and its image is nonzero in the quotient.

\item \textbf{Exercise 5.} Let $X$ be a set with the indiscrete topology $\{\varnothing,X\}$. Show that any sheaf $\mathcal{F}$ on $X$ is uniquely determined by $\mathcal{F}(X)$. \emph{Solution.} The only nonempty open set is $X$ itself, so the only section group is $\mathcal{F}(X)$. There are no proper nonempty open subsets, so no restriction maps need be checked. The sheaf condition is vacuously satisfied. Thus a sheaf on the indiscrete topology is completely determined by the single group $\mathcal{F}(X)$.
