...
\item \textbf{Exercise 1.} State the dimension of the Teichmuller space (deformation space) of smooth curves of genus $g$ for $g=0,1,3$.

\emph{Solution.} For $g=0$: $\dim=0$ (the Riemann sphere is rigid). For $g=1$: $\dim=1$ (one modulus, the $j$-invariant). For $g=3$: $\dim=3g-3=6$.

\item \textbf{Exercise 2.} Describe the Milnor fiber of the plane-curve singularity $y^2-x^3=0$ by considering the deformation $y^2-x^3+t=0$ for small $t\neq0$.

\emph{Solution.} For $t\neq0$ the curve $y^2=x^3-t$ is smooth (the discriminant of $x^3-t$ is nonzero). Its projective closure is a smooth genus-$1$ curve (an elliptic curve), so the Milnor fiber is topologically a once-punctured torus.

\item \textbf{Exercise 3.} Compute the $j$-invariant of the elliptic curve $E:y^2=x^3-x$.

\emph{Solution.} Here $a=-1$, $b=0$, so $c_4=48$, $c_6=0$, $\Delta=c_4^3-c_6^2=48^3=110592\neq0$. The $j$-invariant is $j=c_4^3/\Delta=110592/110592=1728$.

\item \textbf{Exercise 4.} How many independent infinitesimal deformations does a smooth genus-$2$ curve have?

\emph{Solution.} The deformation space has dimension $3g-3=3(2)-3=3$, so there are $3$ independent infinitesimal deformations. The obstruction group $H^2(C,\Theta_C)$ vanishes for dimension reasons on a curve, so all first-order deformations extend.

\item \textbf{Exercise 5.} For the elliptic curve $y^2=x^3+ax$ (with $j\neq0,1728$), compute the rate of change of the $j$-invariant with respect to $a$.

\emph{Solution.} We have $c_4=48a$, $c_6=0$, $\Delta=64a^3$. So $j=c_4^3/\Delta=48^3a^3/(64a^3)=1728$ for all $a\neq0$. Therefore $dj/da=0$: the $j$-invariant is constant along this family, meaning all curves in this family are isomorphic.
