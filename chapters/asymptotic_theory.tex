...
\item[\textbf{Asymptotic equivalence.}] The statement $f(x)\sim g(x)$ means that the ratio $f(x)/g(x)$ tends to 1, so the two functions agree up to a vanishing relative error in the given limit.

\item[\textbf{Big-O notation.}] The claim $f=O(g)$ means that $|f(x)|\leq C|g(x)|$ eventually for some constant $C$; it gives an upper bound on growth rate without requiring closeness.

\item[\textbf{Asymptotic expansion.}] A formal sum of increasingly small correction terms that approximates a function in a limit; the series may diverge, but truncating near the smallest term gives the best approximation.

\item[\textbf{Laplace's method.}] A technique for estimating integrals with a large parameter by localizing the dominant contribution to a neighborhood where the exponent is largest.

\item[\textbf{Singular perturbation.}] A perturbation problem in which directly setting the small parameter to zero changes the order of the differential equation or loses a boundary condition, requiring matched inner and outer solutions.

\item[\textbf{Stirling's formula.}] The asymptotic estimate $n!\sim\sqrt{2\pi n}(n/e)^n$, which replaces a huge exact product by a form suitable for probability and combinatorics.

\item[\textbf{Little-o notation.}] The statement $f=o(g)$ means $f(x)/g(x)\to0$, indicating $f$ is negligible compared to $g$; stronger than big-O.

\item[\textbf{Method of steepest descent.}] A technique for estimating integrals by deforming the contour so the integrand decays most rapidly away from the dominant saddle point.

\item[\textbf{Stationary phase.}] A method for estimating rapidly oscillating integrals by localizing to points where the phase function has zero derivative.

\item[\textbf{Boundary layer.}] A thin region near a boundary where a solution changes rapidly and the outer asymptotic expansion breaks down.
...
\item \textbf{Exercise 1.} Prove that $\binom{2n}{n}\sim \frac{4^n}{\sqrt{\pi n}}$ using Stirling's formula.

\emph{Solution.} We have $\binom{2n}{n}=\frac{(2n)!}{(n!)^2}$. By Stirling, $n!\sim\sqrt{2\pi n}(n/e)^n$. So $(2n)!\sim\sqrt{4\pi n}(2n/e)^{2n}$ and $(n!)^2\sim 2\pi n\,(n/e)^{2n}$. Thus $\binom{2n}{n}\sim\frac{\sqrt{4\pi n}\cdot 4^n n^{2n}/e^{2n}}{2\pi n\cdot n^{2n}/e^{2n}}=\frac{2\sqrt{\pi n}\cdot 4^n}{2\pi n}=\frac{4^n}{\sqrt{\pi n}}$.

\item \textbf{Exercise 2.} Determine whether $3^n+n^3=O(n^3)$, $3^n+n^3\sim 3^n$, or both as $n\to\infty$.

\emph{Solution.} We have $(3^n+n^3)/n^3=(3/n)^n+1\to\infty$, so $3^n+n^3\ne O(n^3)$. However, $(3^n+n^3)/3^n=1+(n/3)^3\cdot 3^{-n}\to 1$, so $3^n+n^3\sim 3^n$. Only the second statement holds.

\item \textbf{Exercise 3.} Use Stirling's formula to find the leading asymptotic behavior of $\binom{n}{n/2}$ for large even $n$.

\emph{Solution.} With $k=n/2$: $\binom{n}{n/2}=\frac{n!}{((n/2)!)^2}$. By Stirling, $n!\sim\sqrt{2\pi n}(n/e)^n$ and $(n/2)!\sim\sqrt{\pi n}(n/(2e))^{n/2}$. So $((n/2)!)^2\sim \pi n\,(n/(2e))^n$. Thus $\binom{n}{n/2}\sim\frac{\sqrt{2\pi n}\,n^n/e^n}{\pi n\,n^n/(2^n e^n)}=\frac{\sqrt{2\pi n}\cdot 2^n}{\pi n}=\sqrt{\frac{2}{\pi n}}\cdot 2^n$.

\item \textbf{Exercise 4.} Show that $\sum_{k=1}^{n}\frac{1}{k}=\log n+\gamma+O(1/n)$, where $\gamma$ is the Euler--Mascheroni constant.

\emph{Solution.} The Euler--Mascheroni constant is defined as $\gamma=\lim_{n\to\infty}\left(\sum_{k=1}^n\frac{1}{k}-\log n\right)$. By the integral test comparison, $\sum_{k=1}^n\frac{1}{k}-\log n=\gamma+O(1/n)$ because $\int_n^{n+1}\frac{dx}{x}-\frac{1}{n+1}=O(1/n^2)$ and the tail converges. The $O(1/n)$ remainder follows from bounding the difference by a telescoping integral.

\item \textbf{Exercise 5.} For $f(x)=x^2 e^{-x}$, find the value of $x$ that maximizes $f$ and determine the leading behavior of $\int_0^\infty x^2 e^{-x}\,dx$ using Laplace's method ideas.

\emph{Solution.} Differentiating: $f'(x)=e^{-x}(2x-x^2)=xe^{-x}(2-x)$, so the maximum is at $x=2$ with $f(2)=4e^{-2}$. The integral is $\Gamma(3)=2!=2$. Laplace's method would estimate this by expanding $\log(x^2 e^{-x})=2\log x -x$ near $x=2$: the second derivative of $2\log x -x$ at $x=2$ is $-2/x^2\big|_{x=2}=-1/2$, giving a Gaussian integral approximation $\int\approx 4e^{-2}\sqrt{2\pi/(1/2)}=4e^{-2}\cdot 2\sqrt{\pi}\approx 2.01$, close to the exact value $2$.
