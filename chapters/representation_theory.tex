...
\item[\textbf{Representation.}] A homomorphism $\rho:G\to GL(V)$ that assigns each element of an abstract group or algebra a linear transformation on a vector space, making the abstract action concrete.
\item[\textbf{Irreducible representation.}] A representation that has no nonzero proper invariant subspace; it is the simplest building block from which all representations are assembled.
\item[\textbf{Character.}] The function $\chi(g)=\operatorname{tr}(\rho(g))$ that records the trace of each matrix in a representation; characters distinguish irreducible representations through their inner products.
\item[\textbf{Maschke's theorem.}] The result that every finite-dimensional representation of a finite group splits into irreducible pieces when the field characteristic does not divide the group order.
\item[\textbf{Unitary representation.}] A representation on a Hilbert space in which every group element acts by a transformation preserving the inner product, central in quantum mechanics.
\item[\textbf{Lie algebra.}] The tangent space of a Lie group at the identity equipped with a bilinear bracket satisfying antisymmetry and the Jacobi identity; it linearizes continuous symmetries.
\item[\textbf{Highest-weight vector.}] A vector in a Lie algebra representation that is annihilated by all positive root spaces and generates the entire representation under the algebra action.
\item[\textbf{Conjugacy class.}] An equivalence class of group elements under $g\sim hgh^{-1}$; characters are constant on conjugacy classes.
\item[\textbf{Character table.}] A square matrix of irreducible characters indexed by conjugacy classes.
\item[\textbf{Schur's lemma.}] A map between irreducible representations is either zero or an isomorphism.
\item[\textbf{Peter--Weyl theorem.}] Matrix coefficients of unitary representations of a compact group span a dense subspace of $L^2$.
\item[\textbf{Tensor product.}] A construction combining two representations into one on the tensor product of their vector spaces.
\end{description}
...
\item \textbf{Exercise 1.} Let $G=\mathbb{Z}/3\mathbb{Z}=\{0,1,2\}$ act on $\mathbb{C}^2$ by $\rho(1)=\begin{pmatrix}\omega&0\\0&\omega^2\end{pmatrix}$ where $\omega=e^{2\pi i/3}$. Compute the character $\chi(g)$ for each $g\in G$. \emph{Solution.} $\chi(0)=\operatorname{tr}(I_2)=2$. $\chi(1)=\omega+\omega^2=-1$. $\chi(2)=\omega^2+\omega=-1$. The character values are $\chi(0)=2$, $\chi(1)=\chi(2)=-1$.

\item \textbf{Exercise 2.} The symmetric group $S_3$ has three conjugacy classes: $\{e\}$, $\{(12),(13),(23)\}$, and $\{(123),(132)\}$. Given the character table:
\[
\begin{array}{c|ccc}
& \{e\} & (12) & (123)\\
\hline
...
\end{array}
\]
Verify that $\chi_1$ and $\chi_3$ are orthogonal using the inner product $\langle\chi_i,\chi_j\rangle=\frac{1}{|G|}\sum_{g}\chi_i(g)\overline{\chi_j(g)}$. \emph{Solution.} $|S_3|=6$. $\langle\chi_1,\chi_3\rangle=\frac{1}{6}[1\cdot 2\cdot 1+3\cdot 1\cdot 0+2\cdot 1\cdot(-1)]=\frac{1}{6}[2+0-2]=0$. The characters are orthogonal.

\item \textbf{Exercise 3.} Consider the permutation representation of $S_3$ acting on $\{1,2,3\}$, sending each permutation to its $3\times 3$ permutation matrix. Decompose this representation into irreducibles using the character table above. \emph{Solution.} The character of the permutation representation is $\chi(e)=3$, $\chi((12))=1$ (one fixed point), $\chi((123))=0$ (no fixed points). Compute $\langle\chi,\chi_1\rangle=\frac{1}{6}[3\cdot 1+3\cdot 1\cdot 1+2\cdot 0\cdot 1]=\frac{6}{6}=1$. $\langle\chi,\chi_2\rangle=\frac{1}{6}[3\cdot 1+3\cdot 1\cdot(-1)+2\cdot 0\cdot 1]=0$. $\langle\chi,\chi_3\rangle=\frac{1}{6}[3\cdot 2+3\cdot 1\cdot 0+2\cdot 0\cdot(-1)]=\frac{6}{6}=1$. So the permutation representation is $\chi_1\oplus\chi_3$.

\item \textbf{Exercise 4.} Let $G=\{e,g\}$ be the cyclic group of order $2$. Classify all $2\times 2$ representations of $G$ over $\mathbb{C}$ up to equivalence. \emph{Solution.} We need $\rho(g)^2=I$. By Schur's lemma and Maschke's theorem, $\rho$ decomposes into irreducibles. The irreducible representations of $G$ are: trivial $\chi_{\text{triv}}(g)=1$ and sign $\chi_{\text{sgn}}(g)=-1$. The possible decompositions of a $2$-dimensional representation are: $\chi_{\text{triv}}\oplus\chi_{\text{triv}}$, $\chi_{\text{triv}}\oplus\chi_{\text{sgn}}$, or $\chi_{\text{sgn}}\oplus\chi_{\text{sgn}}$. Up to equivalence, there are exactly three $2$-dimensional representations.

\item \textbf{Exercise 5.} For the irreducible representation $\chi_3$ of $S_3$ above, verify Schur's lemma by computing $\langle\chi_3,\chi_3\rangle$. \emph{Solution.} $\langle\chi_3,\chi_3\rangle=\frac{1}{6}[|1\cdot\chi_3(e)|^2+3\cdot|\chi_3((12))|^2+2\cdot|\chi_3((123))|^2]=\frac{1}{6}[4+0+2]=\frac{6}{6}=1$. Since the inner product is $1$, $\chi_3$ is irreducible, consistent with Schur's lemma.

\end{enumerate}
\section*{8. References}
\begin{thebibliography}{9}
\bibitem{representation_ref1} J.-P. Serre, \emph{Linear Representations of Finite Groups}, Springer, 1977.
\bibitem{representation_ref2} B. C. Hall, \emph{Lie Groups, Lie Algebras, and Representations}, 2nd ed., Springer, 2015.
\bibitem{representation_ref3} W. Fulton and J. Harris, \emph{Representation Theory: A First Course}, Springer, 1991.
\bibitem{representation_ref4} P. Etingof et al., \emph{Introduction to Representation Theory}, American Mathematical Society, 2011.
\end{thebibliography}
