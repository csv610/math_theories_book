...
\item[\textbf{Test function.}] An infinitely differentiable function with compact support, used as a smooth probe to define the action of a distribution.
\item[\textbf{Distribution.}] A continuous linear functional on test functions that extends the notion of a function, allowing derivatives to be defined for singular objects such as jumps and point masses.
\item[\textbf{Dirac delta.}] The distribution that evaluates a test function at a single point, modelling an idealized point source or instantaneous impulse.
\item[\textbf{Tempered distribution.}] A distribution defined on Schwartz functions (smooth functions that decay faster than any polynomial), important because the Fourier transform extends to this space.
\item[\textbf{Schwartz function.}] A smooth function whose value and all derivatives decrease faster than any inverse power of~$|x|$, forming the natural test-function space for Fourier analysis.
\item[\textbf{Convolution.}] An operation that combines two distributions or functions by integrating one against a shifted copy of the other, with the Dirac delta acting as the identity element.
\item[\textbf{Green function.}] The response of a linear differential operator to a point-source input, used to solve equations by convolving with the source term.
\item[\textbf{Weak derivative.}] The distributional derivative extending differentiation to non-smooth functions via integration by parts.
\item[\textbf{Heaviside step function.}] The function equal to $0$ for $x<0$ and $1$ for $x\geq 0$; its distributional derivative is the Dirac delta.
\item[\textbf{Sobolev space.}] A function space of functions whose weak derivatives up to a given order are $L^p$-integrable.
\end{description}
...
\item \textbf{Exercise 1.} Compute the distributional derivative of $f(x)=|x|$.

\emph{Solution.} For $\varphi\in\mathcal D(\mathbb R)$: $\langle f',\varphi\rangle=-\langle f,\varphi'\rangle=-\int_{-\infty}^{\infty}|x|\varphi'(x)\,dx=-\int_{-\infty}^{0}(-x)\varphi'(x)\,dx-\int_{0}^{\infty}x\varphi'(x)\,dx$. Integrating by parts on each piece: $=-[(-x)\varphi(x)]_{-\infty}^0+\int_{-\infty}^0(-1)\varphi(x)\,dx-[x\varphi(x)]_0^{\infty}+\int_0^{\infty}\varphi(x)\,dx$. Since $\varphi$ has compact support: $=-\int_{-\infty}^0\varphi(x)\,dx+\int_0^{\infty}\varphi(x)\,dx=\langle\operatorname{sgn},\varphi\rangle$. So $|x|'=\operatorname{sgn}(x)$, which equals $2H(x)-1$ where $H$ is the Heaviside function.

\item \textbf{Exercise 2.} Evaluate $\langle\delta'',x^2e^{-x^2}\rangle$.

\emph{Solution.} $\langle\delta'',\varphi\rangle=\varphi''(0)$. For $\varphi(x)=x^2e^{-x^2}$: $\varphi'(x)=(2x-2x^3)e^{-x^2}$, $\varphi''(x)=(2-10x^2+4x^4)e^{-x^2}$. So $\varphi''(0)=2$. Hence $\langle\delta'',x^2e^{-x^2}\rangle=2$.

\item \textbf{Exercise 3.} Compute the distributional derivative of the Heaviside function $H(x-1)$.

\emph{Solution.} $\langle H(x-1)',\varphi\rangle=-\langle H(x-1),\varphi'\rangle=-\int_1^{\infty}\varphi'(x)\,dx$. Since $\varphi$ has compact support: $=-[\varphi(x)]_1^{\infty}=-(0-\varphi(1))=\varphi(1)=\langle\delta(x-1),\varphi\rangle$. So $H(x-1)'=\delta(x-1)$.

\item \textbf{Exercise 4.} Compute the Fourier transform of the constant function $f(x)=1$ as a tempered distribution.

\emph{Solution.} Using the convention $\hat f(\xi)=\int f(x)e^{-i\xi x}\,dx$: we have $\langle\hat 1,\varphi\rangle=\langle 1,\hat\varphi\rangle=\int_{\mathbb R}\hat\varphi(\xi)\,d\xi=2\pi\varphi(0)=2\pi\langle\delta,\varphi\rangle$ by the Fourier inversion theorem. Hence $\hat 1=2\pi\delta$.

\item \textbf{Exercise 5.} Let $f(x)=e^{-|x|}$. Compute the distributional derivative $f'$.

\emph{Solution.} Away from $x=0$, $f'(x)=-e^{-x}$ for $x>0$ and $f'(x)=e^{x}$ for $x<0$, i.e., $f'(x)=-\operatorname{sgn}(x)e^{-|x|}$. Since $f$ is continuous, the distributional derivative equals the classical derivative: $f'=-\operatorname{sgn}(x)\,e^{-|x|}$. Alternatively, $\langle f',\varphi\rangle=-\int_{-\infty}^{\infty}e^{-|x|}\varphi'(x)\,dx$, which integrates by parts to give the same result.
