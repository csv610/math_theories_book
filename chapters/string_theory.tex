...
\paragraph{Why point particles cause trouble}
Quantum mechanics assigns probabilities by summing over all possible paths a particle can take. For a point particle, these paths can intersect at a single spacetime point, producing infinitely many concentrated interactions. In gravity, these intersections become infinitely energetic and the theory gives meaningless infinities. A one-dimensional string spreads its interactions over a two-dimensional surface (the worldsheet), smoothing the infinities. This is the central physical motivation: strings are the simplest objects whose quantum mechanics naturally includes gravity without infinities \cite{string_ref1,string_ref2}.
