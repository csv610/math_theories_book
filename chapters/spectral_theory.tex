...
The finite-dimensional spectral theorem says a Hermitian matrix is unitarily diagonalizable: $U^*DU=\operatorname{diag}(\lambda_1,\ldots,\lambda_n)$. In infinite dimensions, eigenvalues may be replaced by a continuum of ``generalized eigenvalues,'' and a sum becomes an integral. A spectral measure $E$ is a map that assigns, to every Borel subset of $\mathbb{C}$, a projection operator on the Hilbert space. Think of $E(\Delta)$ as the projection onto the part of the space where the operator's ``values'' lie in the set $\Delta$. The operator is then reconstructed as
\[
 T=\int_{\sigma(T)}\lambda\,dE(\lambda),
\]
an infinite-dimensional analogue of $T=\sum\lambda_i P_i$. Functions of $T$ are defined by the same calculus, so expressions such as $e^{tT}$ and square roots can be constructed from the spectrum \cite{spectral_ref3}.
