...
\item \textbf{Exercise 1.} Compute $\rho$ for $g=4$, $r=1$, $d=3$.

\textbf{Solution.} $\rho=4-2(4-3+1)=4-4=0$. A general genus-four curve is therefore expected to have finitely many $g^1_3$ systems.

\item \textbf{Exercise 2.} Why is a hyperelliptic curve special?

\textbf{Solution.} It has a degree-two map to $\mathbb P^1$, a $g^1_2$. If $\rho(g,1,2)<0$, a general curve of genus $g$ does not have this map, so the hyperelliptic curve lies in a special moduli locus.

\item \textbf{Exercise 3.} What does $h^1(D)>0$ mean?

\textbf{Solution.} The divisor is special: it has more sections than the nonspecial Riemann--Roch count predicts. Serre duality interprets $h^1(D)$ as holomorphic differentials constrained by $D$.

\item \textbf{Exercise 4.} What does $\rho<0$ predict?

\textbf{Solution.} A general curve has no $g^r_d$ of the specified degree and dimension. Special curves may still have one.

\item \textbf{Exercise 5.} What is the meaning of a $g^1_d$?

\textbf{Solution.} It is a degree-$d$ linear system of dimension one, usually giving a map from the curve to the projective line.

\end{enumerate}
\section*{8. References}
\begin{thebibliography}{99}
\bibitem{brill_noether_theory_ref1} E. Arbarello, M. Cornalba, P. A. Griffiths, and J. Harris, \emph{Geometry of Algebraic Curves, Volume I}.
\bibitem{brill_noether_theory_ref2} R. Lazarsfeld, \emph{Positivity in Algebraic Geometry I}.
\bibitem{brill_noether_theory_ref3} W. Fulton and R. Lazarsfeld, "On the connectedness of degeneracy loci and special divisors," \emph{Acta Mathematica}.
\end{thebibliography}
