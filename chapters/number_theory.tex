...
Is there a largest prime number? Euclid proved more than two thousand years ago that the answer is no: given any finite list of primes $p_1,\ldots,p_k$, the number $p_1p_2\cdots p_k+1$ is either itself prime or has a prime factor not on the list. This simple argument launches number theory, the study of the integers and their deepest patterns \cite{number_theory_ref1}.

Why do primes matter? Every whole number is either prime or can be built by multiplying primes together, and this decomposition is unique. That fact is the starting point for everything that follows: it tells us which equations have solutions, how messages can be sent securely, and how computers can multiply huge numbers quickly.

A typical question is whether an equation such as $x^2+y^2=z^2$ has solutions in whole numbers, or whether a very large integer is prime. These questions are exact: a numerical experiment can suggest a pattern, but a proof must explain why the pattern continues forever \cite{number_theory_ref1}.

The subject has several connected parts. Elementary number theory studies divisibility, primes, congruences, and Diophantine equations using only algebra. Analytic number theory uses limits and functions to study how primes are distributed. Algebraic number theory enlarges the integers to systems where polynomial equations become easier to understand. Applied number theory underlies public-key cryptography, error-correcting codes, and random-number generation \cite{number_theory_ref1,number_theory_ref2}.
...
\end{historylist}
\begin{quote}\small \textbf{Prerequisite:} High-school arithmetic (division, remainders, prime factorisation). No calculus or proof techniques required for the first two sections.\end{quote}
\section*{3. Theory}
...
\item[\textbf{Prime number.}] A positive integer greater than 1 whose only positive divisors are 1 and itself; primes are the indivisible building blocks of all integers by the fundamental theorem of arithmetic.
\item[\textbf{Congruence.}] A way of saying two integers have the same remainder when divided by a modulus $m$, written $a\equiv b\pmod{m}$; it reduces infinite searches to finite tables of residue classes.
\item[\textbf{Diophantine equation.}] An equation seeking integer or rational solutions, named after Diophantus; solving it requires arithmetic methods rather than the real-number techniques of ordinary algebra.
\item[\textbf{Greatest common divisor (gcd).}] The largest positive integer dividing both $a$ and $b$, computable by the Euclidean algorithm; it determines whether a linear Diophantine equation has solutions.
\item[\textbf{Continued fraction.}] An expression of a real number as a nested fraction that provides unusually good rational approximations and is useful for analyzing quadratic irrationals and Pell equations.
\item[\textbf{Chinese remainder theorem.}] A result stating that simultaneous congruences with pairwise coprime moduli have a unique solution modulo the product of the moduli, enabling large calculations to be split into smaller ones.
\item[\textbf{Fundamental theorem of arithmetic.}] Every integer greater than 1 has a unique prime factorization.
\item[\textbf{Modular arithmetic.}] Arithmetic on residue classes modulo $m$.
\item[\textbf{Euclidean algorithm.}] An algorithm computing the greatest common divisor by repeated division with remainder.
\item[\textbf{Fermat's little theorem.}] If $p$ is prime and $p\nmid a$, then $a^{p-1}\equiv1\pmod{p}$.
\item[\textbf{Euler's totient function.}] The function $\varphi(n)$ counting integers up to $n$ coprime to $n$.
\item[\textbf{Elliptic curve.}] A smooth cubic curve $y^2=x^3+ax+b$ whose points form a group supporting digital signatures.
\item[\textbf{RSA.}] A public-key cryptosystem whose security relies on the difficulty of factoring a large composite $n=pq$.
\end{description}
...
\bibitem{number_theory_ref1} G. H. Hardy and E. M. Wright, \emph{An Introduction to the Theory of Numbers}, 6th edition, Oxford University Press, 2008.
\bibitem{number_theory_ref2} J. H. Silverman, \emph{A Friendly Introduction to Number Theory}, 4th edition, Pearson, 2012.
\bibitem{number_theory_ref3} J. Neukirch, \emph{Algebraic Number Theory}, Springer, 1999.
\end{thebibliography}
