...
\item \textbf{Exercise 1.} In the simply typed lambda calculus, assign a type to each of the following terms: (a) $\lambda x{:}A.\, x$, (b) $\lambda x{:}A.\,\lambda y{:}B.\, x$, (c) $(\lambda x{:}A.\,x)(v)$ where $v : A$. \emph{Solution.} (a) $\lambda x{:}A.\,x$ has type $A \to A$ (the identity function). (b) $\lambda x{:}A.\,\lambda y{:}B.\,x$ has type $A \to B \to A$ (the K combinator, which ignores its second argument). (c) $(\lambda x{:}A.\,x)(v)$ is a well-typed application with $v : A$ and $\lambda x{:}A.\,x : A \to A$, so it has type $A$. It reduces to $v$.

\item \textbf{Exercise 2.}$\beta$-reduction is the computation rule $(\lambda x{:}A.\,M)\,N \to_\beta M[x := N]$. Compute the normal form of $(\lambda x{:}\mathrm{Nat}.\,\lambda y{:}\mathrm{Nat}.\,x)\,3\,4$. \emph{Solution.}$((\lambda x.\,\lambda y.\,x)\,3)\,4 \to_\beta (\lambda y.\,3)\,4 \to_\beta 3$. The normal form is $3$. The term applies a constant function that ignores $y$ and returns $x$, instantiated with $3$.

\item \textbf{Exercise 3.}$\beta$-reduction is confluent (Church--Rosser): if $M$ reduces to both $M_1$ and $M_2$, then there exists $M_3$ such that both $M_1 \to^* M_3$ and $M_2 \to^* M_3$. Verify this for the term $(\lambda x.\,x\,x)((\lambda y.\,y)\,z)$. \emph{Solution.}Two reduction orders: (i) Reduce inner first: $(\lambda y.\,y)\,z \to_\beta z$, giving $(\lambda x.\,x\,x)\,z \to_\beta z\,z$. (ii) Reduce outer first: $(\lambda x.\,x\,x)\,((\lambda y.\,y)\,z) \to_\beta ((\lambda y.\,y)\,z)\,((\lambda y.\,y)\,z) \to_\beta z\,((\lambda y.\,y)\,z) \to_\beta z\,z$. Both paths reach $z\,z$, confirming confluence.

\item \textbf{Exercise 4.} Show that the Church numeral for $2$ is $\overline{2} = \lambda f.\,\lambda x.\,f\,(f\,x)$, and compute $\overline{2}\,\overline{1}$ where $\overline{1} = \lambda f.\,\lambda x.\,f\,x$. \emph{Solution.}$\overline{2}\,\overline{1} = (\lambda f.\,\lambda x.\,f\,(f\,x))\,(\lambda g.\,\lambda y.\,g\,y) \to_\beta \lambda x.\,(\lambda g.\,\lambda y.\,g\,y)\,((\lambda g.\,\lambda y.\,g\,y)\,x) \to_\beta \lambda x.\,\lambda y.\,(\lambda g.\,\lambda y.\,g\,y)\,x\,y \to_\beta \lambda x.\,\lambda y.\,x\,y$. The result is $\overline{1}$, confirming that $2$ applied to $1$ yields $1$ (in the Church encoding, application of numerals corresponds to exponentiation: $\overline{m}\,\overline{n} = \overline{n^m}$, and $1^2 = 1$).

\item \textbf{Exercise 5.} In Martin-Löf type theory, the dependent product $\Pi x{:}A.\,P(x)$ generalizes function types. Give a term of type $\Pi n{:}\mathrm{Nat}.\,\mathrm{Vec}(\mathrm{Bool}, n)$, where $\mathrm{Vec}(T, n)$ is the type of vectors of length $n$ with elements of type $T$. \emph{Solution.}$\lambda n{:}\mathrm{Nat}.\,\mathrm{replicate}(n, \mathrm{false})$ where $\mathrm{replicate}$ constructs a vector of length $n$ by repeating its second argument. The type checks because for each $n : \mathrm{Nat}$, the output has type $\mathrm{Vec}(\mathrm{Bool}, n)$. This function constructs the all-false vector of any specified length, demonstrating the power of dependent types to express precise specifications.
