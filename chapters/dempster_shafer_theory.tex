...
\item[\textbf{Basic belief assignment.}] A function that distributes a total mass of~1 among subsets of possible states, expressing support for sets of outcomes rather than individual ones.
\item[\textbf{Belief.}] The sum of all masses assigned to subsets fully contained in a hypothesis, representing the lower bound of total support for that hypothesis.
\item[\textbf{Plausibility.}] The sum of all masses assigned to subsets that intersect a hypothesis, representing the upper bound of support that does not contradict the hypothesis.
\item[\textbf{Focal element.}] A subset of the state space that receives positive mass in a basic belief assignment.
\item[\textbf{Dempster's rule.}] A combination rule that merges evidence from two independent sources by intersecting their focal sets and rescaling by the nonconflicting mass.
\item[\textbf{Frame of discernment.}] The set of all possible states under consideration, which serves as the universal set over which mass is distributed.
\item[\textbf{Credal set.}] The convex set of all ordinary probability distributions that are compatible with a given belief function, representing the range of probabilities consistent with the evidence.
\item[\textbf{Conflict mass (K).}] The total mass assigned by two sources to pairs of focal sets with empty intersection, measuring disagreement.
\item[\textbf{Normalization.}] The rescaling step in Dempster's rule that removes conflicting mass and redistributes remaining mass to sum to one.
\item[\textbf{Possibility measure.}] A special case of a belief function in which focal sets are nested, related to consonant evidence.
\end{description}
...
\item \textbf{Exercise 1.} Let $X=\{a,b,c\}$ with $m(\{a\})=0.4$, $m(\{b\})=0.3$, $m(\{a,c\})=0.2$, $m(X)=0.1$. Compute $\operatorname{Bel}(\{a,c\})$ and $\operatorname{Pl}(\{a,c\})$.

\emph{Solution.} Belief sums masses of subsets fully contained in $\{a,c\}$: $\operatorname{Bel}(\{a,c\})=m(\{a\})+m(\{a,c\})=0.4+0.2=0.6$. Plausibility sums masses of subsets intersecting $\{a,c\}$: $\operatorname{Pl}(\{a,c\})=m(\{a\})+m(\{a,c\})+m(X)=0.4+0.2+0.1=0.7$.

\item \textbf{Exercise 2.} Two independent sources assign: $m_1(\{a\})=0.6$, $m_1(\{b\})=0.4$; $m_2(\{a,b\})=0.7$, $m_2(\{c\})=0.3$. Combine using Dempster's rule.

\emph{Solution.} Intersections: $\{a\}\cap\{a,b\}=\{a\}$, mass $0.42$; $\{a\}\cap\{c\}=\varnothing$; $\{b\}\cap\{a,b\}=\{b\}$, mass $0.28$; $\{b\}\cap\{c\}=\varnothing$. Conflict: $K=0.18+0.12=0.3$. Normalized: $(m_1\oplus m_2)(\{a\})=0.42/0.7=0.6$; $(m_1\oplus m_2)(\{b\})=0.28/0.7=0.4$.

\item \textbf{Exercise 3.} Given $X=\{a,b,c\}$ with $m(\{a\})=0.4$, $m(\{b\})=0.2$, $m(\{a,b,c\})=0.4$, compute $\operatorname{Bel}(\{a\})$ and $\operatorname{Pl}(\{a\})$.

\emph{Solution.} $\operatorname{Bel}(\{a\})=m(\{a\})=0.4$. $\operatorname{Pl}(\{a\})=m(\{a\})+m(\{a,b,c\})=0.4+0.4=0.8$. Every compatible probability $P$ satisfies $0.4\leq P(\{a\})\leq 0.8$.

\item \textbf{Exercise 4.} Verify that when all focal elements are singletons, Dempster--Shafer reduces to Bayesian probability. Take $X=\{a,b\}$ with $m(\{a\})=0.6$, $m(\{b\})=0.4$.

\emph{Solution.} $\operatorname{Bel}(\{a\})=m(\{a\})=0.6$ and $\operatorname{Pl}(\{a\})=m(\{a\})=0.6$, so $\operatorname{Bel}=\operatorname{Pl}$ for every event. The mass function is identical to a probability distribution with $P(a)=0.6$, $P(b)=0.4$.

\item \textbf{Exercise 5.} Two sources assign: $m_1(\{a\})=0.5$, $m_1(\{b\})=0.5$; $m_2(\{c\})=0.5$, $m_2(\{d\})=0.5$ on frame $X=\{a,b,c,d\}$. Compute the conflict $K$.

\emph{Solution.} Every focal set of $m_1$ ($\{a\}$ and $\{b\}$) is disjoint from every focal set of $m_2$ ($\{c\}$ and $\{d\}$). So $K=m_1(\{a\})m_2(\{c\})+m_1(\{a\})m_2(\{d\})+m_1(\{b\})m_2(\{c\})+m_1(\{b\})m_2(\{d\})=0.25+0.25+0.25+0.25=1$. Dempster's rule is undefined ($1-K=0$).
