...
\item[\textbf{Affine variety.}] The set of common zeros of a collection of polynomial equations in some number of variables, studied together with the polynomial functions defined on it.

\item[\textbf{Projective variety.}] An algebraic set defined inside projective space by homogeneous polynomial equations, which adds ``points at infinity'' so that parallel lines meet and intersection theorems hold without exception.

\item[\textbf{Prime ideal.}] An ideal in a polynomial ring such that whenever a product of two polynomials lies in the ideal, at least one factor already lies in it; algebraically, this corresponds to an irreducible variety.

\item[\textbf{Coordinate ring.}] The ring of polynomial functions restricted to an algebraic set, formed by quotienting the polynomial ring by the ideal of functions vanishing on that set.

\item[\textbf{Morphism.}] A map between algebraic varieties whose coordinate functions are given by polynomials; morphisms are the structure-preserving maps of algebraic geometry.

\item[\textbf{Scheme.}] A generalization of an algebraic variety that replaces points with prime ideals of a ring and adds a structure sheaf, allowing one to treat number rings, finite fields, and geometric families in a single framework.

\item[\textbf{Gr\"obner basis.}] A specially chosen generating set for a polynomial ideal that makes algebraic computations---such as solving systems or testing membership---systematic, analogous to Gaussian elimination for linear systems.

\item[\textbf{Birational equivalence.}] A relation between two varieties when there exist rational maps in both directions that are inverse to each other wherever they are defined; birational geometry studies properties that survive this kind of transformation.

\item[\textbf{Zariski topology.}] The topology on an algebraic set whose closed sets are zero loci of ideals; it is coarser than the Euclidean topology and remembers polynomial relations rather than distance.

\item[\textbf{Nullstellensatz.}] Hilbert's theorem that over an algebraically closed field, the ideal of polynomials vanishing on the zero set of an ideal equals its radical, bridging algebraic sets and ideal theory.

\item[\textbf{Regular function.}] A function on an algebraic variety that is locally a ratio of polynomials with nonzero denominator; on an affine variety, regular functions form the coordinate ring.

\item[\textbf{Divisor.}] A formal sum of codimension-one subvarieties with integer coefficients, recording where a rational function vanishes or has poles.
...
\item \textbf{Exercise 1.} Find the coordinate ring of the curve $V(x^2+y^2-1)\subset\mathbb{R}^2$.

\emph{Solution.} The coordinate ring is $\mathbb{R}[x,y]/(x^2+y^2-1)$. In this ring the relation $y^2=1-x^2$ holds, so every polynomial function on the circle can be reduced to a polynomial of the form $a(x)+b(x)y$ where $a,b\in\mathbb{R}[x]$.

\item \textbf{Exercise 2.} Let $I=(x^2-y,\,xy-1)\subset\mathbb{Q}[x,y]$. Compute a Gr\"obner basis for $I$ with respect to the lexicographic order $x>y$.

\emph{Solution.} From $x^2=y$ we get $xy=1$, so $x\cdot y=1$ and substituting $x^2=y$ gives $x\cdot x^2=1$, i.e.\ $x^3=1$. Then $y=x^2$ and $xy-1=0$. A Gr\"obner basis is $\{x^3-1,\,y-x^2\}$. One checks that the leading terms $x^3$ and $y$ generate the leading term ideal: any polynomial in $I$ reduces to zero via these two generators.

\item \textbf{Exercise 3.} Find all common solutions of $x^2+y^2=5$ and $x+y=3$ over $\mathbb{R}$.

\emph{Solution.} Substitute $y=3-x$ into the first equation: $x^2+(3-x)^2=5$, so $2x^2-6x+9=5$, giving $2x^2-6x+4=0$, i.e.\ $x^2-3x+2=0$. Hence $x=1$ or $x=2$, yielding the two points $(1,2)$ and $(2,1)$.

\item \textbf{Exercise 4.} Determine whether $y-x^2=0$ and $y-x^3=0$ intersect at the origin with multiplicity 2.

\emph{Solution.} Setting $y=x^2$ in $y-x^3=0$ gives $x^2-x^3=0$, so $x^2(1-x)=0$. The root $x=0$ has multiplicity 2 (from the factor $x^2$), while $x=1$ has multiplicity 1. By B\'ezout's theorem, the total intersection count with multiplicity is $2+1=3$, matching the product of degrees $2\cdot 3=6$ only after accounting for intersections at infinity; in the affine chart, the intersection at the origin has multiplicity 2.

\item \textbf{Exercise 5.} Find the ideal $I(S)$ of all polynomials vanishing on $S=\{(0,0),(1,0),(0,1)\}\subset\mathbb{R}^2$.

\emph{Solution.} We need $f(0,0)=f(1,0)=f(0,1)=0$. The polynomial $x(x-1)$ vanishes at $(0,0)$ and $(1,0)$ but not at $(0,1)$, so it is not in $I(S)$. One checks that $I(S)=(xy,\,x^2-x,\,y^2-y)$. Indeed $xy$ vanishes at all three points, $x^2-x=x(x-1)$ vanishes at the first two and at $(0,1)$ since $0\cdot(0-1)=0$, and $y^2-y$ vanishes at the first and third. These generate the vanishing ideal.
