...
\item \textbf{Exercise 1.} Compute $H^k(S^1\vee S^1;\mathbb{R})$ for all $k$, where $S^1\vee S^1$ is the wedge of two circles.

\emph{Solution.} Use Mayer--Vietoris with $U$ a neighborhood of the first circle and $V$ a neighborhood of the second, so $U\simeq S^1$, $V\simeq S^1$, $U\cap V\simeq\{\text{point}\}$. The long exact sequence gives $H^0\cong\mathbb{R}$ (connected), $H^1\cong\mathbb{R}^2$ (one generator per circle), and $H^k=0$ for $k\geq 2$ (the space is one-dimensional).

\item \textbf{Exercise 2.} Use the Mayer--Vietoris sequence to compute $H^k(T^2;\mathbb{R})$ for the torus $T^2=S^1\times S^1$.

\emph{Solution.} Decompose $T^2=U\cup V$ where $U$ is a tubular neighborhood of $S^1\times\{p\}$ and $V$ is a tubular neighborhood of $S^1\times\{q\}$ for two distinct points. Then $U\simeq S^1$, $V\simeq S^1$, $U\cap V\simeq S^1\sqcup S^1$. The Mayer--Vietoris sequence yields $H^0(T^2)\cong\mathbb{R}$, $H^1(T^2)\cong\mathbb{R}^2$, $H^2(T^2)\cong\mathbb{R}$, and $H^k(T^2)=0$ for $k\geq 3$.

\item \textbf{Exercise 3.} Show that the $2$-form $\omega=x\,dy\wedge dz+y\,dz\wedge dx+z\,dx\wedge dy$ on $S^2$ is closed but not exact.

\emph{Solution.} Compute $d\omega=dx\wedge dy\wedge dz+dy\wedge dz\wedge dx+dz\wedge dx\wedge dy=3\,dx\wedge dy\wedge dz$, but on $S^2$ (a $2$-dimensional manifold) there are no $3$-forms, so $d\omega=0$ and $\omega$ is closed. By Stokes' theorem, if $\omega=d\alpha$ then $\int_{S^2}\omega=\int_{S^2}d\alpha=\int_{\partial S^2}\alpha=0$ (since $\partial S^2=\varnothing$). However, $\int_{S^2}\omega=3\cdot\text{Vol}(S^2)=12\pi\neq 0$, so $\omega$ is not exact.

\item \textbf{Exercise 4.} Use the long exact sequence of the pair $(D^2,S^1)$ to compute $H^k(D^2,S^1;\mathbb{R})$ for all $k$.

\emph{Solution.} Since $D^2$ is contractible, $H^k(D^2)\cong\mathbb{R}$ for $k=0$ and $0$ otherwise. The long exact sequence gives $H^0(D^2,S^1)\cong\mathbb{R}$ and $H^k(D^2,S^1)\cong H^{k-1}(S^1)$ for $k\geq 1$. Hence $H^1(D^2,S^1)\cong\mathbb{R}$, $H^k(D^2,S^1)=0$ for $k\geq 2$.

\item \textbf{Exercise 5.} Let $\alpha\in H^1(T^2;\mathbb{R})$ and $\beta\in H^1(T^2;\mathbb{R})$ be the classes dual to the two generating circles. Compute $\alpha\cup\beta$ in $H^2(T^2;\mathbb{R})$.

\emph{Solution.} Since $T^2$ is a closed oriented surface, $H^2(T^2;\mathbb{R})\cong\mathbb{R}$, generated by the class of the fundamental class. By Poincar\'e duality, $\alpha\cup\beta$ evaluates to $\pm 1$ on the fundamental class. Graded commutativity gives $\alpha\cup\beta=-\beta\cup\alpha$, so $\alpha\cup\beta$ is a nonzero element of $H^2(T^2;\mathbb{R})$, hence a generator. In particular, $\alpha\cup\beta\neq 0$, distinguishing $T^2$ from $S^1\vee S^1$ which has trivial cup product.
