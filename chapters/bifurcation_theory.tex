...
\item \textbf{Exercise 1.} Analyze the equilibria of $\dot x=rx-x^3$.

\textbf{Solution.} Equilibria are $x=0$ and, for $r\geq0$, $x=\pm\sqrt r$. The derivative is $r-3x^2$. Thus zero is stable for $r<0$ and unstable for $r>0$, while the two nonzero branches are stable for $r>0$. This is a supercritical pitchfork.

\item \textbf{Exercise 2.} Classify the equilibria of $\dot x=\lambda-x^2$.

\textbf{Solution.} For $\lambda>0$, $x=\pm\sqrt\lambda$; the positive branch has derivative $-2\sqrt\lambda<0$ and is stable, the negative branch unstable. At $\lambda=0$ they merge, and for $\lambda<0$ there are none: a saddle-node.

\item \textbf{Exercise 3.} What eigenvalue condition signals a local continuous-time bifurcation?

\textbf{Solution.} At least one eigenvalue of the Jacobian has real part zero. A zero eigenvalue gives a steady-state bifurcation; a nonzero purely imaginary pair gives the linear signal for a Hopf bifurcation. Nondegeneracy conditions are still required.

\item \textbf{Exercise 4.} Why is a homoclinic bifurcation global?

\textbf{Solution.} It involves an entire orbit connecting a saddle to itself and therefore the global stable and unstable manifolds, not just the eigenvalues at the saddle. The change cannot be confined to an arbitrarily small neighborhood.

\item \textbf{Exercise 5.} What does codimension two mean for a Bogdanov--Takens point?

\textbf{Solution.} Two independent conditions must be met, so one generally needs two parameters to reach the degenerate point. Nearby parameter values may display several codimension-one curves such as saddle-node and Hopf branches.

\end{enumerate}
\section*{8. References}
\begin{thebibliography}{99}
\bibitem{bifurcation_theory_ref1} S. H. Strogatz, \emph{Nonlinear Dynamics and Chaos}.
\bibitem{bifurcation_theory_ref2} Y. A. Kuznetsov, \emph{Elements of Applied Bifurcation Theory}.
\bibitem{bifurcation_theory_ref3} V. I. Arnold, \emph{Geometrical Methods in the Theory of Ordinary Differential Equations}.
\bibitem{bifurcation_theory_ref4} S. Wiggins, \emph{Introduction to Applied Nonlinear Dynamical Systems and Chaos}.
\end{thebibliography}
