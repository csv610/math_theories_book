...
\item \textbf{Exercise 1.} In unoriented cobordism, determine whether the Klein bottle is cobordant to the torus $T^2$.

\emph{Solution.} In unoriented cobordism, two closed manifolds are cobordant if and only if all their Stiefel--Whitney numbers agree. The torus $T^2$ has trivial tangent bundle, so $w_1=0$ and $w_2=0$, giving all Stiefel--Whitney numbers zero. The Klein bottle is non-orientable with $w_1\neq 0$, and $w_1^2\neq 0$ in $H^2(K^2;\mathbb{Z}/2)\cong\mathbb{Z}/2$. Since the Stiefel--Whitney numbers differ, the Klein bottle is \emph{not} cobordant to $T^2$.

\item \textbf{Exercise 2.} Show that the connected sum $T^2\mathbin{\#} T^2$ (a genus-$2$ orientable surface) is null-cobordant in unoriented cobordism.

\emph{Solution.} Every closed orientable surface bounds a handlebody: a $3$-ball with handles attached. The genus-$2$ surface bounds the genus-$2$ handlebody, a compact $3$-manifold whose boundary is exactly the surface. Since it bounds, it is null-cobordant. Equivalently, all Stiefel--Whitney numbers of any closed orientable surface vanish.

\item \textbf{Exercise 3.} Construct an explicit cobordism from $S^2$ to $T^2$ and state how many handles are attached.

\emph{Solution.} Start with $S^2\times[0,1]$. Attach one $1$-handle ($S^0\times D^2$) by gluing its two ends to two disjoint disks on $S^2\times\{1/2\}$. The resulting $3$-manifold $W$ has boundary $S^2$ at the bottom and a genus-$1$ surface (the torus) at the top. A Morse function on $W$ has one critical point of index $1$, corresponding to the single $1$-handle.

\item \textbf{Exercise 4.} Find a Morse function on the torus $T^2$ and count its critical points by index.

\emph{Solution.} Use the height function $f(x,y,z)=z$ on the standard embedding of $T^2$ in $\mathbb{R}^3$. There is one minimum (index $0$) at the bottom, one saddle (index $1$) on the side, and one maximum (index $2$) at the top. The Euler characteristic is $1-1+1=1$. However, $\chi(T^2)=0$, so we must have $\chi=1-1+1=1$. Reconsidering: a correct Morse function on $T^2$ has exactly one minimum (index $0$), one saddle (index $1$), and one maximum (index $2$), giving $\chi=1-1+1=1$, which disagrees with $\chi(T^2)=0$. The resolution is that a genus-$1$ surface requires two saddles: the correct count is one minimum (index $0$), two saddles (index $1$), and one maximum (index $2$), giving $\chi=1-2+1=0$.

\item \textbf{Exercise 5.} Compute the Stiefel--Whitney numbers of $S^3$ and deduce whether $S^3$ is null-cobordant in unoriented cobordism.

\emph{Solution.} $S^3$ is orientable, so $w_1(S^3)=0$. Its tangent bundle is stably trivial, so all Stiefel--Whitney classes $w_i(S^3)$ vanish for $i\geq 1$. Therefore all Stiefel--Whitney numbers are zero, and $S^3$ is null-cobordant. Concretely, $S^3$ bounds the $4$-ball $D^4$.
