...
\item \textbf{Exercise 1.} Compute $\operatorname{ex}(8,K_4)$ using Tur\'an's theorem.

\emph{Solution.} Tur\'an's theorem gives $\operatorname{ex}(n,K_{r+1})=|E(T(n,r))|$ where $T(n,r)$ is the balanced complete $r$-partite graph. For $K_4$-free graphs on $8$ vertices: $r=3$, parts of sizes $\lceil8/3\rceil=3$, $\lceil8/3\rceil=3$, $\lfloor8/3\rfloor=2$. Number of edges: $3\cdot3+3\cdot2+3\cdot2=9+6+6=21$. Equivalently, $\operatorname{ex}(8,K_4)=\lfloor8^2(1-1/3)/2\rfloor=\lfloor64/3\rfloor=21$.

\item \textbf{Exercise 2.} Find $\operatorname{ex}(6,K_3)$ by exhibiting an extremal graph and verifying with Mantel's theorem.

\emph{Solution.} Mantel's theorem: $\operatorname{ex}(6,K_3)=\lfloor36/4\rfloor=9$. The balanced complete bipartite graph $K_{3,3}$ has $3\times3=9$ edges and no triangle. Any graph on $6$ vertices with $10$ edges must contain a triangle.

\item \textbf{Exercise 3.} Prove that $R(3,3)=6$.

\emph{Solution.} Lower bound: color the edges of $K_5$ by putting the cycle $C_5$ in red and the complementary $C_5$ in blue. Neither color contains a triangle, so $R(3,3)>5$. Upper bound: in any 2-coloring of $K_6$, pick a vertex $v$. It has $5$ incident edges, so at least $3$ have the same color, say red, connecting to vertices $a,b,c$. If any edge among $\{a,b,c\}$ is red, we get a red triangle. Otherwise, all three edges among $\{a,b,c\}$ are blue, forming a blue triangle. Hence $R(3,3)\leq6$.

\item \textbf{Exercise 4.} A graph has maximum degree $\Delta=5$ and is not $K_6$ or an odd cycle. What is the minimum number of colors that always suffices to properly color it?

\emph{Solution.} Brooks' theorem states $\chi(G)\leq\Delta(G)$ for any connected graph that is not a complete graph or odd cycle. Here $\Delta=5$ and $G\neq K_6$, so $\chi(G)\leq5$. Five colors always suffice.

\item \textbf{Exercise 5.} Compute the chromatic number of the complement $\overline{C_5}$ of the $5$-cycle.

\emph{Solution.} $C_5$ has vertices $\{1,2,3,4,5\}$ with edges between consecutive vertices. In $\overline{C_5}$, each vertex is adjacent to the two non-neighbors in $C_5$. The complement is isomorphic to $C_5$ itself (the map $i\mapsto 2i\pmod{5}$ gives an isomorphism). Since $C_5$ is an odd cycle, $\chi(C_5)=3$. Therefore $\chi(\overline{C_5})=3$.
