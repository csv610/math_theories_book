...
\item \textbf{Exercise 1.} Find the weights expressing $(2,1)$ as a convex combination of $(0,0)$, $(4,0)$, and $(0,3)$.

\emph{Solution.} Write $(2,1)=a(0,0)+b(4,0)+c(0,3)$ with $a+b+c=1$. The coordinates give $4b=2$ and $3c=1$, so $b=1/2$, $c=1/3$, and $a=1/6$. All weights are nonnegative and sum to one.

\item \textbf{Exercise 2.} Explain why a point in the interior of a line segment is not extreme.

\emph{Solution.} If $z$ lies strictly between $x$ and $y$, then $z=\lambda x+(1-\lambda)y$ for some $0<\lambda<1$ with $x\ne y$. This is exactly the forbidden type of decomposition in the definition of an extreme point.

\item \textbf{Exercise 3.} Give two different representations of the centre of the unit square using its vertices.

\emph{Solution.} The centre $(1/2,1/2)$ is the average of opposite vertices $(0,0)$ and $(1,1)$, and also the average of $(1,0)$ and $(0,1)$. Hence the representing measure is not unique.

\item \textbf{Exercise 4.} Why does a probability measure naturally replace finite weights in an infinite-dimensional convex set?

\emph{Solution.} A finite sum may not contain enough extreme points to build the desired element. A probability measure can distribute mass over infinitely many extreme points, and integration is the limiting form of a weighted sum. Compactness and continuity make this average mathematically well-defined.

\item \textbf{Exercise 5.} What extra property makes a convex set a Choquet simplex?

\emph{Solution.} It must have a unique representing probability measure on its extreme points for every point of the set. This is stronger than merely having at least one representation.
