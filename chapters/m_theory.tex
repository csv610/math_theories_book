...
\item \textbf{Exercise 1.} A string is compactified on a circle of radius $R=2\ell_s$. Using T-duality ($R'=\ell_s^2/R$), find the dual radius and state whether the two theories are physically equivalent.

\emph{Solution.} $R'=\ell_s^2/(2\ell_s)=\ell_s/2$. T-duality identifies the physics of a string on a circle of radius $R$ with that on radius $R'=\ell_s^2/R$. The two descriptions are equivalent: momenta on the $R$-circle correspond to winding on the $R'$-circle and vice versa.

\item \textbf{Exercise 2.} A particle moves on a circle of radius $R=2$ in Planck units. Compute the Kaluza--Klein mass gap (the mass of the first excited state above the ground state).

\emph{Solution.} Kaluza--Klein masses are $m_n=n/R$ for $n\in\mathbb{Z}$. The ground state has $n=0$ and mass $0$. The first excited states have $n=\pm 1$, giving mass $m_1=1/2$. The mass gap is $\Delta m=1/2$.

\item \textbf{Exercise 3.} Type IIA string theory is compactified on a $6$-torus $T^6$. What is the effective spacetime dimension of the resulting theory?

\emph{Solution.} The uncompactified Type IIA theory lives in $10$ dimensions. Compactifying on $T^6$ removes $6$ dimensions, leaving an effective theory in $10-6=4$ dimensions.

\item \textbf{Exercise 4.} The BPS bound states $M\geq|Z|$, where $M$ is mass and $Z$ is the central charge. A BPS state has $Z=5$ in appropriate units. What is its mass?

\emph{Solution.} For a BPS state the bound is saturated: $M=|Z|=5$.

\item \textbf{Exercise 5.} When $11$-dimensional supergravity is reduced on a circle of radius $R$, the string coupling of the resulting Type IIA theory satisfies $g_s\propto R^{3/2}$. If the radius is doubled, by what factor does the string coupling change?

\emph{Solution.} $g_s\propto R^{3/2}$, so doubling $R$ gives a factor of $2^{3/2}=2\sqrt{2}\approx 2.83$. The coupling becomes stronger, consistent with the fact that large $R$ in M-theory corresponds to strong coupling in Type IIA.
