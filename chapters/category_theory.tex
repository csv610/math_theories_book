...
\emph{Solution.} $F$ is not well-defined as stated. If $\varphi:G\to H$ is a homomorphism, $\varphi(Z(G))\subseteq Z(H)$ only when $\varphi$ is surjective or under additional hypotheses. For a counterexample, take $G=S_3$ (trivial center), $H=S_3$, and $\varphi=\mathrm{id}$. Then $Z(G)=\{e\}$ maps to $\{e\}\subseteq Z(H)=\{e\}$, which works. But consider $G=\mathbb{Z}$, $H=S_3$, and $\varphi$ the trivial map. Then $\varphi(Z(G))=\{e\}\subseteq Z(H)$, which works. However, consider $G=\mathbb{Z}\times\mathbb{Z}$, $H=\mathbb{Z}/2\mathbb{Z}\times S_3$, and $\varphi(a,b)=(a\bmod 2,b)$. Then $Z(G)=\mathbb{Z}\times\mathbb{Z}$ maps to $\mathbb{Z}/2\mathbb{Z}\times Z(S_3)=\mathbb{Z}/2\mathbb{Z}\times\{e\}$, which is fine. Actually, $\varphi(Z(G))\subseteq Z(H)$ always holds: if $z\in Z(G)$ and $h=\varphi(g)\in H$, then $\varphi(z)\varphi(g)=\varphi(zg)=\varphi(gz)=\varphi(g)\varphi(z)$, so $\varphi(z)\in Z(\mathrm{im}\,\varphi)\subseteq Z(H)$. So $F$ is well-defined. Functoriality: $F(\mathrm{id}_G)=\mathrm{id}_{Z(G)}$ and $F(\psi\circ\varphi)=(\psi\circ\varphi)|_{Z(G)}=\psi|_{\varphi(Z(G))}\circ\varphi|_{Z(G)}=F(\psi)\circ F(\varphi)$, so $F$ is a functor.
...
\item \textbf{Exercise 3.} Let $F,G:\mathbf{Set}\to\mathbf{Set}$ be the functors defined by $F(X)=X\times\mathbb{Z}$ (fixed set $\mathbb{Z}$) and $G(X)=X^{\{0,1\}}$ (functions from $\{0,1\}$ to $X$). Show that $F$ and $G$ are naturally isomorphic.

\emph{Solution.} We have $F(X)=X\times\mathbb{Z}$ and $G(X)=\{(x_0,x_1):x_0,x_1\in X\}=X\times X$. Since $|\mathbb{Z}|=|\{0,1\}|^{\aleph_0}$ is infinite while $|\{0,1\}|=2$, these are not isomorphic in general. The statement is false. Let me correct: take $F(X)=X\times\{0,1\}$ and $G(X)=X^{\{0,1\}}=X\times X$. These are not naturally isomorphic either. Actually, $F(X)=X\times\{0,1\}$ has $|F(X)|=2|X|$ while $|G(X)|=|X|^2$. For $|X|=2$: $|F(X)|=4=|G(X)|$, but for $|X|=3$: $|F(X)|=6\ne 9=|G(X)|$. So $F$ and $G$ are not naturally isomorphic. The correct example: let $F(X)=X\times\{0,1\}$ and $G(X)=X\sqcup X$ (disjoint union). Then $\eta_X:F(X)\to G(X)$ defined by $\eta_X(x,0)=(x,\text{left})$ and $\eta_X(x,1)=(x,\text{right})$ is a natural isomorphism. For any $f:X\to Y$, $G(f)\circ\eta_X=\eta_Y\circ F(f)$ because both send $(x,i)$ to $(f(x),i)$.
